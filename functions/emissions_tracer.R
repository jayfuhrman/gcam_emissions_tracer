...
energy_water_distributor <- function(prj){
  ...
  input <- input_combine %>% filter(sector == "CO2 removal") %>%
    ...
    mutate(input = if_else(str_detect(input,"delivered gas"),"delivered gas",input),
           sector = if_else(str_detect(sector,"delivered gas"),"delivered gas",sector),
           subsector = if_else(str_detect(subsector,"delivered gas"),"delivered gas",subsector),

           input = if_else(str_detect(input,"delivered coal"),"delivered coal",input),
           sector = if_else(str_detect(sector,"delivered coal"),"delivered coal",sector),
           subsector = if_else(str_detect(subsector,"delivered coal"),"delivered coal",subsector),

           input = if_else(str_detect(input,"wholesale gas"),"wholesale gas",input),
           sector = if_else(str_detect(sector,"wholesale gas"),"wholesale gas",sector),
           subsector = if_else(str_detect(subsector,"wholesale gas"),"wholesale gas",sector),

           input = if_else(str_detect(input,"delivered biomass"),"delivered biomass",input),
           sector = if_else(str_detect(sector,"delivered biomass"),"delivered biomass",sector),
           subsector = if_else(str_detect(subsector,"delivered biomass"),"delivered biomass",subsector),

           input = if_else(str_detect(input,"elect_td_h2|elect_td_en"),"elect_td_ind",input),
           sector = if_else(str_detect(sector,"elect_td_h2|elect_td_en"),"elect_td_ind",sector),
           subsector = if_else(str_detect(subsector,"elect_td_h2|elect_td_en"),"elect_td_ind",subsector),

           input = if_else(str_detect(input,"refined liquids bunkers|refined liquids transport"),"refined liquids enduse",input),
           sector = if_else(str_detect(sector,"refined liquids bunkers|refined liquids transport"),"refined liquids enduse",sector),
           subsector = if_else(str_detect(subsector,"refined liquids bunkers|refined liquids transport"),"refined liquids enduse",subsector),

           input = if_else(str_detect(input,"refined liquids feedstocks"),"refined liquids industrial",input),
           sector = if_else(str_detect(sector,"refined liquids feedstocks"),"refined liquids industrial",sector),
           subsector = if_else(str_detect(subsector,"refined liquids feedstocks"),"refined liquids industrial",subsector)) %>%
    group_by(scenario,region,year,sector,subsector,input,Units) %>%
    summarize(value = sum(value)) %>%
    ungroup()
  ...
  enduse_sectors <- dplyr::setdiff(sectors$sector, unique(c(sectors$input)))
  ...
  transformation_sectors <- c("delivered biomass", "delivered coal", "delivered coal industry","delivered coal feedstocks",
                              ...
                              "elect_td_h2","elect_td_H2","elect_td_en",
  ...
  sumcheck2 <- left_join(tmp2,tmp3, by = c("scenario","region","year","input")) %>%
    mutate(diff = value.x - value.y) %>%
    filter(abs(diff) >= 1e-6)
  ...
  # Repeating fuel distribution with delivered biomass and coal
    group_by(scenario, region, year) %>%
    group_modify(~passthru_remove(., c( "delivered biomass", "delivered coal","delivered coal industry","delivered coal feedstocks","delivered biomass industry")), .keep=TRUE) %>%
    ungroup()
  ...
  #and drop any remaining rows where sectors consume themselves as inputs
   ...
   sumcheck3 <- left_join(tmp3,tmp4, by = c("scenario","region","year","input")) %>%
     mutate(diff = value.x - value.y) %>%
     filter(abs(diff) >= 1e-6)
  ...
  transform_sectors <- c("H2 enduse","H2 retail delivery","H2 retail dispensing","H2 wholesale dispensing","H2 industrial",
                         "elect_td_bld", "elect_td_trn", "elect_td_ind","elect_td_h2","elect_td_H2","elect_td_en",
  ...
  sumcheck4 <- left_join(tmp4,tmp5, by = c("scenario","region","year","input")) %>%
    mutate(diff = value.x - value.y) %>%
    filter(abs(diff) >= 1e-6)


  if (nrow(sumcheck4) > 0){
    print("Sumcheck mismatch introduced at Transformation sectors removed as inputs to other transformations")
  }
...
fuel_distributor <- function(prj){
  ...
  transformation_sectors <- c("delivered biomass", "delivered coal", "delivered coal industry","delivered coal feedstocks",
                              "delivered gas","delivered gas transport","delivered gas h2",
                              "elect_td_bld", "elect_td_ind", "elect_td_trn","elect_td_h2","elect_td_en",
                              "H2 retail delivery","H2 retail dispensing","H2 industrial","H2 wholesale dispensing","H2 enduse",
                              "refined liquids enduse", "refined liquids industrial","refined liquids transport","refined liquids bunkers","refined liquids feedstocks",
                              "wholesale gas", "wholesale gas feedstocks","traditional biomass", "district heat")
...
final_fuel_CO2_disag <- function(all_emissions){
  ...
  trn_inputs_by_subsector <-
    ...
    filter(subsector %in% transport_sectors,
           input %in% c('refined liquids enduse','delivered gas','refined liquids industrial','refined liquids transport','refined liquids bunkers')) %>%
    rename(enduse = subsector,
           PrimaryFuelCO2Coef.name = input) %>%
    left_join(ccoef_mapping, by = c('PrimaryFuelCO2Coef.name')) %>%
    mutate(transformation = if_else(str_detect(PrimaryFuelCO2Coef.name,'refined liquids'),'refining',
                                    if_else(str_detect(PrimaryFuelCO2Coef.name,'delivered gas'),'gas processing',NA_character_)),
  ...
  if (round(sum(remaining_industry_CO2$value) - sum(remaining_industry_disag$value), 0) != 0){
    print("Remaining industry CO2 emission disaggregation: Total emissions from 1990 to 2100 do NOT match.")
    print("percent difference between raw GCAM output data and initial disaggregation pass emissions is:")
...
lifecycle_CO2_emiss_phase_disag <- function(data_input){
  ...
  inputs_by_subsector %>%
    ...
    mutate(c_input = value * PrimaryFuelCO2Coef) %>%
    select(-Units) %>%
    filter(!(PrimaryFuelCO2Coef.name %in% c('elect_td_en','elect_td_ind','H2 industrial', "global solar resource", "onshore wind resource","energy-constraint"))) -> upstream_inputs_by_subsector
  ...
  df_lifecycle_disag <- bind_rows(df_upstream, df_downstream, df_for_bindback) %>%
    ...
    mutate(phase = if_else(ghg == 'CO2' & transformation %in% c('district heat','H2 central production','H2 wholesale dispensing','H2 enduse','H2 production'),'midstream',phase),
           phase = if_else(ghg != 'CO2' & direct == "district heat","midstream",phase))
...
emissions <- function(CO2, CO2_bio, resource_CO2, nonCO2, LUC,
  ...
  fuel_tracing_adj_h2 <-
    fuel_tracing %>%
    mutate(transformation = if_else(transformation %in% c("H2 industrial", "H2 LDV", "H2 MHDV", "H2 wholesale delivery", "LH2","H2 retail delivery") & elec_for_H2 == FALSE, "H2 central production", transformation)) %>%
    group_by(scenario, region, year, primary, transformation, enduse, Units, elec_for_H2) %>%
    summarise(value = sum(value, na.rm = TRUE))
  ...
  fuel_tracing_adj <-
    ...
    group_by(scenario, region, year, primary) %>%
    mutate(ratio_enduse_in_primary = value / sum(value),
           sum_ratio_enduse_in_primary = sum(ratio_enduse_in_primary)) %>%
    # Get ratio of enduse in transformation (disaggregated by primary category)
    group_by(scenario, region, year, transformation) %>%
    mutate(ratio_enduse_in_transformation = value / sum(value),
           sum_ratio_enduse_in_transformation = sum(ratio_enduse_in_transformation)) %>%
    ungroup()
  ...
  if (round(original_emissions - calculated_emissions_rus,0) != 0){
    ...
    original_emissions_grouped <- ghg_co2eq %>%
      group_by(scenario,region,year,ghg) %>%
    ...
    write_csv(initial_disag_left_join_original,'DEBUG_joined_initial_disag.csv')
  ...
  if (round(original_emissions - calculated_emissions, 0) != 0){
    ...
    print(paste0('Final Fuel nonCO2 Disaggregation Sum: ',sum((all_emissions3%>%filter(year >= 2005))$value, na.rm = TRUE)))
    print(paste0('Lifecycle phase Disaggregation Sum: ',sum((all_emissions4%>%filter(year >= 2005))$value, na.rm = TRUE)))
    print(paste0('Final Sum: ',sum((all_emissions_region%>%filter(year >= 2005))$value, na.rm = TRUE)))
